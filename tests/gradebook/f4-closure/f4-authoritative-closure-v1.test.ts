...
const AUDIT_CATEGORY_FAMILIES = ['estrutural', 'cadastral', 'nota', 'cálculo', 'origem', 'tempo'] as const;

describe('F4 authoritative closure V1 — ROADMAP bullet-by-bullet', () => {
  it('preserves the seven historical F4 requirements without redefining current diagnostic retention', () => {
    const roadmap = source('docs/gradebook/history/pre-final-1/ROADMAP.md');
  ...
  it('bullet 1 — stable academic keys prevent duplicate streams without using technical record ids', () => {
    const gradeEntryStream = {
      kind: 'grade-entry', studentId: 'student:f4-closure:synthetic',
      enrollmentId: 'enrollment:f4-closure:synthetic', assessmentComponentId: 'assessment:f4-closure:synthetic',
    } as unknown as AcademicRecordStreamV1;
    const stableKey = academicRecordStreamKeyV1(gradeEntryStream);
    const associationStream = {
      logicalSourceId: 'logical-source:f4-closure:synthetic', academicRecordStream: gradeEntryStream, stableKey,
    ...
    const plannerTests = source('tests/gradebook/reconciliation/idempotency/import-reconciliation-v1.test.ts');
    expect(planner).toContain("'duplicate-incoming-stream'");
    expect(planner).toContain('academicRecordsSemanticallyEqualV1');
    expect(plannerTests).toContain('keeps identical and renamed hashes free of academic association versions');
  });
  ...
  it('bullet 2 — source, academic values and source associations keep append-only versions', () => {
    ...
    expect(executorTests).toContain('commits source, academic records and associations in one transaction');
  ...
  it('bullet 3 — FOI PARA / ESTAVA NO preserves trajectory and the current population without double counting', () => {
    ...
    expect(sourceContract).toContain('`FOI PARA XX` na origem e `ESTAVA NO XX` no destino representam movimento explícito dentro do ano.');
    expect(sourceContract).toContain('Notas anteriores replicadas no destino não podem causar dupla contagem.');
    expect(sourceTests).toContain('ID-002/ID-003: preserva FOI PARA/ESTAVA NO e notas replicadas nas duas posições');
  ...
  it('bullet 4 — promotion/rejection remains isolated in the official planner and transactional executor', () => {
    const plannerTests = source('tests/gradebook/reconciliation/idempotency/import-reconciliation-v1.test.ts');
    const executor = source('server/gradebook/application/import/execution/execute-import-change-plan-v1.ts');
    const executorTests = source('tests/gradebook/reconciliation/execution/execute-import-change-plan-v1.test.ts');
    expect(AUDIT_WORKSPACE_PROMOTION_POLICY_V1).toEqual({
      eligibilitySource: 'existing-import-change-plan', planner: 'planImportReconciliation',
      executor: 'executeImportChangePlan', workspacePromotionOperation: 'forbidden', promotionRequestPayload: 'forbidden',
    });
  ...
  it('bullet 7 — critical errors cannot masquerade as full success and valid files stay independently promotable', () => {
    const planner = source('server/gradebook/application/import/import-reconciliation-v1.ts');
    const plannerTests = source('tests/gradebook/reconciliation/idempotency/import-reconciliation-v1.test.ts');
    const importContracts = source('shared/gradebook-contracts/imports/import-contract-v1.ts');
    ...
    expect(plannerTests).toContain('expect(plan.promotionRequest.approvedImportFileIds).toEqual([validFile.id])');
    expect(importContracts).toContain('criticalErrorCount');
    expect(importContracts).toContain("'partially-approved'");
  });

  it('keeps historical closure semantics and the provider-independent boundaries', () => {
    const planner = source('server/gradebook/application/import/import-reconciliation-v1.ts');
    const workspace = source('server/gradebook/application/audit-workspace/audit-workspace-v1.ts');
