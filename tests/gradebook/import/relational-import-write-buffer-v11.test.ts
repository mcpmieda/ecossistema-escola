...
describe('relational import write buffer v11', () => {
  it('groups current note mutations before the next read', async () => {
  ...
  it('rejects any attempt to recreate retired note history', async () => {
    const { database } = fakeDatabase();
    const buffered = createBufferedRelationalImportDatabaseV11(database);
    await expect(
    ...
    ).rejects.toThrow('gradebook-note-history-retired');
  });
