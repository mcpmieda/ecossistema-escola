...
describe('BN-DEC-033 parallel recovery', () => {
  it.each([1, 2, 3] as const)('does not require either assessment in term %s', (term) => {
    for (const [av1, av2] of [[null, 200], [200, null], [null, null]] as const) {
      ...
      expect(result.quantitativeConsideredMilli).toBe((av1 ?? 0) + (av2 ?? 0) + 7000);
      expect(result.rawMilli).toBe((av1 ?? 0) + (av2 ?? 0) + 19000);
  ...
  it.each([1, 2, 3] as const)('uses strict institutional quantitative limits in term %s', (term) => {
    ...
    expect(calculate(term, cutoff - 1, 0, 12000, 0).parallelApplicable).toBe(true);
    for (const value of [cutoff, cutoff + 1]) {
      const result = calculate(term, value, 0, 12000, 0);
      expect(result.parallelApplicable).toBe(false);
      expect(result.rawMilli).toBe(value);
    }
  });

  it.each([1, 2, 3] as const)('dispenses with PARA at the unrounded term limit in term %s', (term) => {
    ...
    expect(below.rawMilli).toBe(cutoff - 1 + 8000);
    for (const value of [cutoff, cutoff + 1]) {
      const result = calculate(term, 2000, null, 8000, value - 2000);
      expect(result.parallelApplicable).toBe(false);
      expect(result.quantitativeConsideredMilli).toBe(2000);
      expect(result.rawMilli).toBe(value);
      expect(result.warnings.map((warning) => warning.code)).toContain('parallel-present-when-not-applicable');
    }
  });

  it('adds a superior PARA once, without discarding the original quantitative points', () => {
    const result = calculate(2, 200, null, 7000, 12000);
    expect(result.parallelApplicable).toBe(true);
    expect(result.quantitativeOriginalMilli).toBe(200);
    expect(result.quantitativeConsideredMilli).toBe(7200);
    expect(result.rawMilli).toBe(19200);
  ...
  it('uses PARA only when strictly greater than AV1 plus AV2', () => {
    ...
    expect(result.quantitativeConsideredMilli).toBe(6001);
    expect(result.rawMilli).toBe(15001);
  });

  it('does not test eligibility against a total that already includes PARA', () => {
    const result = calculate(1, 4000, 2000, 8000, 11000);
    expect(result.parallelApplicable).toBe(true);
    expect(result.rawMilli).toBe(25000);
  ...
  it('does not substitute malformed configured maxima for institutional limits', () => {
    const instruments = facts(1, 7000, 0, 8000, 0).map((fact) =>
