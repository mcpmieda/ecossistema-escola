...
it('mounts the actual overview with pedagogical KPIs, participation and collapsed technical information', () => {
  ...
  expect(screen.queryByRole('meter', { name: 'Cobertura dos instrumentos' })).toBeNull();
...
it('opens quality on demand and exposes a focused information explanation', async () => {
  render(<PerformanceAnalyticsWorkspaceV6 {...props()} />);
  const user = userEvent.setup();
