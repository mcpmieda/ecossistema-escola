...
import { adminQueryV1, type AdminResponseV1, type AdminQueryV1 } from '../../../shared/student-portal-contracts/admin-v1';
...
const context = (capability = 'platform.settings.write') => ({ actorId: ACTOR, tenantId: TENANT, requestId: crypto.randomUUID(), authenticatedAt: new Date().toISOString(), capability });
...
async function query<T extends AdminQueryV1['operation']>(operation: T, extra: Record<string, unknown> = {}) {
  return state(await api.query(context(), { contractVersion: 1, operation, scope: CLASS, page: {}, ...extra }), operation);
}
const command = (operation: string, expectedVersion: number, extra: Record<string, unknown> = {}) => ({ contractVersion: 1, operation, expectedVersion, idempotencyKey: crypto.randomUUID(), ...extra });
const version = async () => (await query('accounts', { scope: accountScope() })).items[0]!.version;
const accountCommand = async (operation: string, extra: Record<string, unknown> = {}) => command(operation, await version(), { accountId: accounts[0]!.id, ...extra });
async function setting(value: Record<string, unknown>) {
  const current = await query('settings', { scope: SCHOOL });
  return state(await api.command(context(), command('settings-set', current.settings.version, { scope: SCHOOL, value, acknowledgeImmediateEffect: true })), 'committed');
}
...
beforeAll(async () => {
  ...
  accounts = (await pg.query<{ id: string; student: number }>('SELECT id,gradebook_student_id AS student FROM student_portal.account ORDER BY gradebook_student_id')).rows;
  const run = async <R extends Record<string, unknown>>(target: Pick<PGlite, 'query'>, query: string, parameters: readonly unknown[] = []) => {
    calls++;
    if (failReceipt && query.includes('INSERT INTO student_portal.operation_receipt')) throw new Error('synthetic-receipt-failure');
  ...
  api = new PortalAdminApiV1(sql, { tenantId: TENANT, cryptoPort: cryptography, qrKeyVersion: 1, pepperVersion: 1, cursorSecret: SECRET });
}, 30_000);
beforeEach(async () => {
  failReceipt = false;
  ...
  for (const account of accounts) await pg.query(`UPDATE student_portal.account SET gradebook_student_id=$2,closed_at=NULL,
    auth_state='pending-activation',eligibility='eligible',blocked=false,version=0,security_version=0,pin_version=0 WHERE id=$1`, [account.id, account.student]);
...
describe('private administrative API with real persistence', () => {
  it('rejects absent, forged, stale, wrong-tenant or insufficient contexts before database access', async () => {
    ...
    for (const invalid of [null, {}, { ...context(), role: 'ADMINISTRADOR' }, { ...context(), tenantId: ACTOR },
      { ...context(), authenticatedAt: new Date(Date.now() - 301_000).toISOString() }, { ...context(), authenticatedAt: new Date(Date.now() + 1000).toISOString() }]) {
      expect((await api.query(invalid, request)).state).toBe('forbidden');
    }
    expect((await api.command(context('platform.settings.read'), command('block', 0, { accountId: accounts[0]!.id, blocked: true, confirmed: true }))).state).toBe('forbidden');
    expect(calls).toBe(0);
    expect((await api.query(context('platform.settings.read'), request)).state).toBe('accounts');
    expect((await api.query(context(), { ...request, role: 'ADMINISTRADOR' })).state).toBe('invalid-request');
    expect((await api.query(context(), { ...request, page: { limit: 101 } })).state).toBe('invalid-request');
  });
  ...
  it('lists only the requested accounts, state, name and class, with bounded scoped keyset cursors', async () => {
    ...
    const base = { contractVersion: 1, operation: 'accounts', scope: CLASS, page: { limit: 1, cursor: first.nextCursor } };
    expect((await api.query({ ...context(), actorId: TENANT }, base)).state).toBe('invalid-request');
    expect((await api.query(context(), { ...base, scope: SCHOOL })).state).toBe('invalid-request');
    expect((await api.query(context(), { ...base, page: { limit: 1, cursor: 'a'.repeat(43) } })).state).toBe('invalid-request');
    ...
    expect((await query('accounts', { scope: { ...CLASS, classId: 910002 } })).items).toHaveLength(0);
    expect(JSON.stringify(first)).not.toMatch(/credential|password|birth|token|verifier|PRIVATE TEACHER/);
  });
  ...
  it('issues, reprints, rotates and resets with account CAS, distinct semantics and explicit confirmation', async () => {
    ...
    expect((await api.command(context(), { ...rotate, idempotencyKey: crypto.randomUUID() })).state).toBe('conflict');
    expect((await api.command(context(), { ...rotate, expectedVersion: await version(), confirmed: false })).state).toBe('invalid-request');
    for (const operation of ['password-reset', 'account-reset']) {
      state(await api.command(context(), await accountCommand(operation, { confirmed: true })), 'committed');
      expect((await query('accounts', { scope: accountScope() })).items[0]!.state).toBe(operation === 'password-reset' ? 'reset-required' : 'pending-activation');
    }
    for (const blocked of [true, false]) {
      state(await api.command(context(), await accountCommand('block', { blocked, confirmed: true })), 'committed');
      expect((await query('accounts', { scope: accountScope() })).items[0]!.blocked).toBe(blocked);
    }
    expect((await api.command(context(), command('qr-issue', 0, { accountId: crypto.randomUUID() }))).state).toBe('forbidden');
  });
  ...
  it('prints all three batch modes atomically, retains active QRs and safely replays a committed reference', async () => {
    ...
    for (const mode of ['qr-only', 'qr-name', 'qr-name-class']) {
      ...
      const input = command('qr-batch', snapshot.scopeVersion, { classId: CLASS.classId, accountIds: accounts.map((account) => account.id), mode, confirmed: true });
      ...
      previous = urls;
      expect(result.cards.every((card) => ('name' in card) === (mode !== 'qr-only') && ('classLabel' in card) === (mode === 'qr-name-class'))).toBe(true);
      expect(JSON.stringify(result)).not.toMatch(/birth|password|verifier|pin|tokenHash/);
      expect((await api.command(context(), { ...input, mode: mode === 'qr-only' ? 'qr-name' : 'qr-only' })).state).toBe('conflict');
    }
    expect((await pg.query<{ n: number }>('SELECT count(*)::integer AS n FROM student_portal.qr_credential')).rows[0]!.n).toBe(2);
  });

  it('rejects another class or duplicate ids and rolls back the entire QR batch when the receipt cannot commit', async () => {
    const snapshot = await query('accounts');
    const input = command('qr-batch', snapshot.scopeVersion, { classId: CLASS.classId, accountIds: accounts.map((account) => account.id), confirmed: true });
    expect((await api.command(context(), { ...input, classId: 910002 })).state).toBe('forbidden');
    expect((await api.command(context(), { ...input, accountIds: [accounts[0]!.id, accounts[0]!.id.toUpperCase()] })).state).toBe('invalid-request');
    failReceipt = true;
    expect((await api.command(context(), input)).state).toBe('unavailable');
    failReceipt = false;
    expect((await pg.query<{ n: number }>('SELECT count(*)::integer AS n FROM student_portal.qr_credential')).rows[0]!.n).toBe(0);
  ...
  it('exposes separate birth/account/scope versions and dispatches write, clear and resumable per-item batches', async () => {
    ...
    expect(initial.items.every((item) => item.year === null && item.accountVersion === 0 && item.version === 0)).toBe(true);
    const item = initial.items.find((item) => item.accountId === accounts[0]!.id)!;
    state(await api.command(context(), command('birth-write', item.accountVersion, { item: { action: 'set', accountId: item.accountId,
      expectedVersion: item.version, year: '2001', confirmation: 'unconfirmed-test' } })), 'committed');
    const updated = await query('birth-years');
    expect(updated.items.find((item) => item.accountId === accounts[0]!.id)).toMatchObject({ year: '2001', confirmation: 'unconfirmed-test', accountVersion: 1, version: 1 });
    const batch = command('birth-batch', updated.scopeVersion, { classId: CLASS.classId, expectedCount: updated.items.length,
      items: updated.items.map((item) => ({ action: 'clear', accountId: item.accountId, expectedVersion: item.version })), confirmed: true });
  ...
  it('lists sessions without hashes and revokes only the selected account session', async () => {
    ...
    for (let index = 0; index < 2; index++) await pg.query(`INSERT INTO student_portal.session(id,account_id,token_hash,security_version,expires_at,persistent)
      VALUES ($1,$2,$3,0,statement_timestamp()+interval '1 day',false)`, [ids[index], accounts[index]!.id, `synthetic-only-hash-${index}`.padEnd(43, 'x')]);
    ...
    state(await api.command(context(), command('sessions-revoke', list.version, { scope: accountScope(), sessionId: ids[1], confirmed: true })), 'committed');
    expect((await query('sessions', { scope: SCHOOL })).items.every((item) => item.revokedAt === null)).toBe(true);
    state(await api.command(context(), command('sessions-revoke', list.version, { scope: accountScope(), sessionId: ids[0], confirmed: true })), 'committed');
  ...
  it('dispatches settings inheritance and publication using their actual exposed CAS values', async () => {
    ...
    state(await api.command(context(), command('settings-set', initial.settings.version, { scope: CLASS, value: { showPartials: true }, acknowledgeImmediateEffect: true })), 'committed');
    const changed = await query('settings');
    expect(changed.settings.value.showPartials).toBe(true);
    state(await api.command(context(), command('settings-inherit', changed.settings.version, { scope: CLASS, keys: ['showPartials'] })), 'committed');
    ...
    const at = (offset: number) => new Date(Math.floor(Date.now() / 1000) * 1000 + offset * 86400_000).toISOString();
    await setting({ accessEnabled: true, allowedPeriods: ['T1'], calendar: { ...current, yearStartsAt: at(-60), t1EndsAt: at(-50), t2EndsAt: at(-40),
      t3EndsAt: at(-30), recoveriesStartAt: at(-20), finalDisclosureAt: at(-10), yearEndsAt: at(30), disclosure: { mode: 'single', at: at(-10), periods: ['T1'] } } });
    ...
    for (const operation of ['publish', 'publish-update', 'unpublish']) {
      ...
      const extra = operation === 'unpublish' ? { confirmed: true } : { targetDataVersion: t1.availableRevision };
      state(await api.command(context(), command(operation, t1.version, { scope: accountScope(), period: 'T1', ...extra })), 'committed');
      await new PublicationJobsV1(sql).run();
    }
  });
  ...
  it('paginates audit tuples precisely, masks lists and restricts raw IP to write capability within ninety days', async () => {
    ...
    const request = { contractVersion: 1, operation: 'audit-detail', scope: SCHOOL, page: {}, eventId: first.items[0]!.eventId };
    ...
    await pg.query("UPDATE student_portal.audit_event SET occurred_at=statement_timestamp()-interval '91 days' WHERE event_id=$1", [request.eventId]);
    expect(state(await api.query(context(), request), 'audit-detail')).toMatchObject({ ip: null, ipExpiresAt: null });
    expect((await api.query(context(), { ...request, scope: { ...CLASS, classId: 910002 } })).state).toBe('forbidden');
  });

  it('rejects expired cursors, irrelevant query filters and unavailable databases without returning partial data', async () => {
    const parsed = adminQueryV1.parse({ contractVersion: 1, operation: 'accounts', scope: CLASS, page: { limit: 1 } });
    const cursor = await new AdminCursorV1(SECRET).next(parsed, ACTOR, new Date(Date.now() - 301_000), accounts[0]!.id);
    expect((await api.query(context(), { ...parsed, page: { limit: 1, cursor } })).state).toBe('invalid-request');
    expect((await api.query(context(), { ...parsed, operation: 'settings', nameSearch: 'ignored' })).state).toBe('invalid-request');
    const broken: StudentPortalPostgresSqlV1 = { unsafe: async () => { throw new Error('private-db-error'); }, begin: async () => { throw new Error('private-db-error'); } };
    const other = new PortalAdminApiV1(broken, { tenantId: TENANT, cryptoPort: cryptography, qrKeyVersion: 1, pepperVersion: 1, cursorSecret: SECRET });
  ...
  it('reports sanitized health and requires school preview for idempotent link closure without resetting the BN', async () => {
    ...
    expect(await query('health')).toMatchObject({ status: 'normal' });
    expect((await api.query(context(), { contractVersion: 1, operation: 'links-preview', scope: CLASS, page: {} })).state).toBe('forbidden');
    const before = (await pg.query<{ n: number }>('SELECT count(*)::integer AS n FROM gradebook.aluno')).rows[0]!.n;
    const preview = await query('links-preview', { scope: SCHOOL });
    const input = command('links-close', preview.version, { academicYear: 2026, previewToken: preview.previewToken, expectedCount: preview.count, confirmed: true });
    expect((await api.command(context(), { ...input, confirmed: false })).state).toBe('invalid-request');
    const first = state(await api.command(context(), input), 'committed');
    const replay = state(await api.command(context(), input), 'committed');
    expect(replay.operationId).toBe(first.operationId);
    expect(replay.version).toBe(first.version);
    expect((await query('accounts', { scope: SCHOOL })).items.every((item) => item.link === null)).toBe(true);
    expect((await pg.query<{ n: number }>('SELECT count(*)::integer AS n FROM gradebook.aluno')).rows[0]!.n).toBe(before);
    expect((await api.command(context(), { ...input, expectedCount: preview.count + 1 })).state).toBe('conflict');
  });
  ...
  it('accepts the full one-hundred-card boundary with bounded SQL rather than per-account round trips', async () => {
    ...
    const list = await query('accounts', { scope: { ...CLASS, classId: 910003 }, page: { limit: 100 } });
    expect(list.items).toHaveLength(100);
    expect(list.nextCursor).toBeNull();
    const input = command('qr-batch', list.scopeVersion, { classId: 910003,
      accountIds: list.items.map((item) => item.accountId), mode: 'qr-only', confirmed: true });
    const before = calls;
    const result = state(await api.command(context(), input), 'qr');
