...
const cryptoPort = new PortalCryptoV1(new Map([[1, new Uint8Array(32).fill(11)]]), new Map([[1, new Uint8Array(32).fill(22)]]));
let pg: PGlite;
...
beforeAll(async () => {
  ...
  const run = async <R extends Record<string, unknown>>(target: Pick<PGlite, 'query'>, query: string, parameters: readonly unknown[] = []) => {
    if (failAudit && query.includes('INSERT INTO student_portal.audit_event')) throw new Error('synthetic-db-failure');
...
async function version() { return Number((await pg.query<{ version: string }>('SELECT version::text FROM student_portal.account WHERE id=$1', [accountId])).rows[0]!.version); }
async function command(operation: string, extra: Record<string, unknown> = {}) {
  return { contractVersion: 1, operation, accountId, expectedVersion: await version(), idempotencyKey: id(), ...extra };
}
async function birthWrite(action = 'set', year = '2001', confirmation = 'confirmed') {
  const rows = (await pg.query<{ version: string }>('SELECT version::text FROM student_portal.account_access_data WHERE account_id=$1', [accountId])).rows;
  return birth.write(ACTOR, { contractVersion: 1, operation: 'birth-write', expectedVersion: await version(), idempotencyKey: id(),
    item: { accountId, expectedVersion: Number(rows[0]?.version ?? 0), action, ...(action === 'set' ? { year, confirmation } : {}) } });
}
...
async function activate(persistent = false) {
  const result = await auth.activate({ contractVersion: 1, challenge: await proof(), password: '123456', confirmation: '123456', keepConnected: persistent }, id());
...
beforeEach(async () => {
  ...
  const now = Date.now();
  await policy.mutate(ACTOR, { contractVersion: 1, operation: 'settings-set', scope: SCHOOL, expectedVersion: current.version,
    idempotencyKey: id(), acknowledgeImmediateEffect: true, value: { accessEnabled: true, calendar: { ...current.value.calendar,
...
describe('auth with real schema, policies, birth service and scrypt', () => {
  it('requires QR/PIN, commits activation once, stores only hashes and requires password for an active account', async () => {
    expect(await auth.challenge({ contractVersion: 1, qr }, id())).toMatchObject({ state: 'credential-required', next: 'pin' });
    const challenge = await proof();
    const request = { contractVersion: 1, challenge, password: '123456', confirmation: '123456', keepConnected: false };
    ...
    expect(results.filter((item) => 'state' in item && item.state === 'unauthenticated')).toHaveLength(1);
    const result = results.find((item) => 'token' in item)!;
    if (!('token' in result)) throw new Error('synthetic-no-token');
    expect(await sessions.read(result.token, id())).toMatchObject({ state: 'authenticated', persistent: false });
    ...
    expect(JSON.stringify((await pg.query('SELECT * FROM student_portal.auth_challenge')).rows)).not.toContain(challenge);
    expect(await auth.challenge({ contractVersion: 1, qr, pin: '2001' }, id())).toMatchObject({ state: 'credential-required', next: 'password' });
    expect(await auth.login({ contractVersion: 1, qr, password: '200100', keepConnected: false }, id())).toMatchObject({ state: 'unauthenticated' });
  });
  ...
  it('records failures across service instances, requires risk after three, blocks after five and expires by DB time', async () => {
    ...
    for (let i = 0; i < 3; i++) expect(await (i % 2 ? other : auth).login(bad, id())).toMatchObject({ state: 'unauthenticated' });
    expect(await auth.challenge({ contractVersion: 1, qr }, id())).toMatchObject({ next: 'risk' });
    expect(await other.login({ ...bad, password: '123456' }, id())).toMatchObject({ state: 'unauthenticated' });
    expect(Number((await pg.query<{ failures: number }>('SELECT failures FROM student_portal.auth_attempt')).rows[0]!.failures)).toBe(3);
    expect(await auth.login({ ...bad, riskToken: 'synthetic-valid-risk' }, id())).toMatchObject({ state: 'unauthenticated' });
    expect(await other.login({ ...bad, riskToken: 'synthetic-valid-risk' }, id())).toMatchObject({ state: 'rate-limited', retryAfterSeconds: expect.any(Number) });
    expect(await auth.login({ ...bad, password: '123456', riskToken: 'synthetic-valid-risk' }, id())).toMatchObject({ state: 'rate-limited' });
    await pg.exec("UPDATE student_portal.auth_attempt SET blocked_until=statement_timestamp()-interval '1 second'");
  ...
  it('birth correction/clear rejects old proofs without revoking a valid session, and unconfirmed birth cannot activate', async () => {
    ...
    expect(await auth.activate({ contractVersion: 1, challenge: old, password: '123456', confirmation: '123456', keepConnected: false }, id())).toMatchObject({ state: 'unauthenticated' });
    await birthWrite();
    const signed = await activate();
    await birthWrite('clear');
    expect(await new SessionServiceV1(sql, cryptoPort).read(signed.token, id())).toHaveProperty('state', 'authenticated');
  ...
  it('reprints identically, rotates QR while preserving password, resets password with same QR and resets account with new QR', async () => {
    ...
    expect((await qrService.command(ACTOR, reprint)).qr).toBe(qr);
    const rotated = await qrService.command(ACTOR, await command('qr-regenerate', { confirmed: true }));
    expect(rotated.qr).not.toBe(qr);
    expect(await sessions.read(signed.token, id())).toBeNull();
    expect(await auth.login({ contractVersion: 1, qr, password: '123456', keepConnected: false }, id())).toMatchObject({ state: 'unauthenticated' });
    qr = rotated.qr!;
    expect(await auth.login({ contractVersion: 1, qr, password: '123456', keepConnected: false }, id())).toHaveProperty('token');
    await qrService.command(ACTOR, await command('password-reset', { confirmed: true }));
    ...
    expect((await pg.query('SELECT birth_year FROM student_portal.account_access_data')).rows).toEqual([{ birth_year: 2001 }]);
  });
  ...
  it('revalidates current policy and shorter TTL, then logout remains effective across service restarts', async () => {
    ...
    const current = await policy.read(SCHOOL);
    await policy.mutate(ACTOR, { contractVersion: 1, operation: 'settings-set', scope: SCHOOL, expectedVersion: current.version,
      idempotencyKey: id(), acknowledgeImmediateEffect: true, value: { risk: { ...current.value.risk, persistentSeconds: 60, shortSeconds: 60 } } });
    await pg.exec("UPDATE student_portal.session SET created_at=statement_timestamp()-interval '61 seconds'");
    expect(await sessions.read(signed.token, id())).toBeNull();
    const login = await auth.login({ contractVersion: 1, qr, password: '123456', keepConnected: false }, id());
  ...
  it('caps issuance at year end and disables current sessions immediately when access is switched off', async () => {
    ...
    await policy.mutate(ACTOR, { contractVersion: 1, operation: 'settings-set', scope: SCHOOL, expectedVersion: current.version,
      idempotencyKey: id(), acknowledgeImmediateEffect: true, value: { calendar: { ...current.value.calendar, yearEndsAt } } });
    const signed = await activate(true);
    expect(Date.parse(signed.body.expiresAt)).toBe(Date.parse(yearEndsAt));
    const changed = await policy.read(SCHOOL);
    await policy.mutate(ACTOR, { contractVersion: 1, operation: 'settings-set', scope: SCHOOL, expectedVersion: changed.version,
      idempotencyKey: id(), acknowledgeImmediateEffect: true, value: { accessEnabled: false } });
    expect(await sessions.read(signed.token, id())).toBeNull();
    expect(await auth.login({ contractVersion: 1, qr, password: '123456', keepConnected: true }, id())).toMatchObject({ state: 'unauthenticated' });
  });

  it('revokes all sessions in a current class with CAS and idempotent receipt', async () => {
    const signed = await activate();
    const second = await auth.login({ contractVersion: 1, qr, password: '123456', keepConnected: true }, id());
    ...
    const input = { contractVersion: 1, operation: 'sessions-revoke', scope, expectedVersion: snapshot.version, idempotencyKey: id(), confirmed: true };
  ...
  it('rolls back activation and proof consumption on DB failure, and never sends a cookie before commit', async () => {
    const challenge = await proof();
    const input = { contractVersion: 1, challenge, password: '123456', confirmation: '123456', keepConnected: false };
    failAudit = true;
    const response = await servePortalAuthV1(new Request(`${ORIGIN}/api/student/auth/activate`, {
  ...
  it('keeps tokens exclusively in secure cookies and rejects cross-origin, duplicate cookies and oversized JSON', async () => {
    const call = (path: string, input: unknown, headers: Record<string, string> = {}) => servePortalAuthV1(new Request(`${ORIGIN}${path}`, {
      method: 'POST', headers: { 'Content-Type': 'application/json', ...headers }, body: JSON.stringify(input),
    }), 'production', ORIGIN, auth, sessions);
    const input = { contractVersion: 1, challenge: await proof(), password: '123456', confirmation: '123456', keepConnected: false };
    const response = await call('/api/student/auth/activate', input);
    expect(response.status).toBe(200);
    const cookie = response.headers.get('Set-Cookie')!;
    expect(cookie).toMatch(/^__Host-student_portal_session=[A-Za-z0-9_-]{43}; Path=\/; Secure; HttpOnly; SameSite=Strict$/u);
    const token = cookie.split(';')[0]!.split('=')[1]!;
    expect(JSON.stringify(await response.json())).not.toContain(token);
    expect(response.headers.get('Cache-Control')).toBe('no-store');
    const duplicated = await servePortalAuthV1(new Request(`${ORIGIN}/api/student/session`, { headers: { cookie: `${cookie.split(';')[0]}; ${cookie.split(';')[0]}` } }), 'production', ORIGIN, auth, sessions);
    expect(duplicated.status).toBe(401);
    expect((await call('/api/student/auth/login', {}, { origin: 'https://attacker.invalid' })).status).toBe(403);
    expect((await call('/api/student/auth/login', { data: 'x'.repeat(8192) })).status).toBe(413);
    const persistent = await call('/api/student/auth/login', { contractVersion: 1, qr, password: '123456', keepConnected: true });
    expect(persistent.headers.get('Set-Cookie')).toContain('; Expires=');
    expect(persistent.headers.get('Set-Cookie')).not.toContain('Domain');
    const logout = await call('/api/student/auth/logout', { contractVersion: 1 }, { cookie: cookie.split(';')[0]! });
  ...
  it('rejects tampering, expired challenges, bad password formats, stale CAS and idempotency payload changes', async () => {
    ...
    expect(await auth.challenge({ contractVersion: 1, qr: parts.join('.'), pin: '2001' }, id())).toMatchObject({ state: 'unauthenticated' });
    const challenge = await proof();
    ...
    expect(await auth.activate({ contractVersion: 1, challenge, password: '123456', confirmation: '123456', keepConnected: false }, id())).toMatchObject({ state: 'unauthenticated' });
    await expect(auth.login({ contractVersion: 1, qr, password: '12345 ', keepConnected: false }, id())).rejects.toThrow();
    const cmd = await command('block', { blocked: true, confirmed: true });
    await qrService.command(ACTOR, cmd);
    await expect(qrService.command(ACTOR, { ...cmd, blocked: false })).rejects.toThrow('idempotency-conflict');
    await expect(qrService.command(ACTOR, { ...cmd, idempotencyKey: id() })).rejects.toThrow('version-conflict');
  });
});
