...
- `0006_import_diagnostic_treatment_v1.sql`: extensão aditiva #674 com reconhecimento/anotação append-only separados do snapshot corrente, ator server-side e ACL `SELECT, INSERT`; sem backfill, resolução manual ou DML acadêmico. Ainda não aplicada em produção.
...
`0006` permanece somente na branch/PR da #674. Não adicioná-la ao plano de recovery integrado nem aplicá-la remotamente antes da autorização explícita de schema. O gate futuro exige backup, preflight das 29 tabelas, migration transacional, postflight da 30ª relação vazia/ACL mínima e atualização correspondente do procedimento de recuperação. Ver [`RELATIONAL_AUDIT_TREATMENT_V1.md`](../../docs/gradebook/RELATIONAL_AUDIT_TREATMENT_V1.md).
