...
function parseRequest(value: unknown): Required<BenchmarkRequestV1> {
  ...
  if (!Number.isInteger(size) || (size as number) < 1 || (size as number) > POSTGRES_BENCHMARK_MAX_SIZE_V1)
    throw new HttpError(400, 'Invalid benchmark size');
  if (!Number.isInteger(changed) || (changed as number) < 0 || (changed as number) > (size as number))
...
export const onRequestPost: PagesFunction<BenchmarkEnvV1> = async (context: Context) => {
  ...
  try {
    ...
    if (cause instanceof AuthenticationError || cause instanceof AuthorizationError) {
      return Response.json({ state: 'not-authorized' }, { status: cause.status, headers: { 'Cache-Control': 'no-store' } });
    }
    throw cause;
  }
  ...
  if (!binding?.connectionString) throw new HttpError(503, 'Postgres benchmark unavailable');
  ...
  const benchmarkId = `synthetic:${crypto.randomUUID()}`;
