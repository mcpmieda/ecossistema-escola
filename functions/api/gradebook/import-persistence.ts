...
function statusFor(value: GradebookImportPersistenceResponseV9): number {
  switch (value.state) {
    case 'applied': case 'no-changes': case 'review-required': return 200;
    case 'blocked': case 'conflict': return 409;
...
async function handle(request: Request, env: RuntimeEnv): Promise<Response> {
  ...
  if (inspection !== 'ready') {
    return response({
      transportVersion:9,state:'invalid-request',
...
export const onRequest: PagesFunction<RuntimeEnv> = async (context) => {
  try {
    ...
    const value: GradebookImportPersistenceResponseV9 = status === 401 || status === 403
      ? {transportVersion:9,state:'not-authorized'} : status < 500
    ...
    if (status >= 500) console.error(JSON.stringify({message:'gradebook_relational_import_failed'}));
