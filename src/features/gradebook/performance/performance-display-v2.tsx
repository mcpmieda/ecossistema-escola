...
export function GradeValue({ cell, prominent = false }: { readonly cell: PerformanceCellV2; readonly prominent?: boolean }) {
  const tone = cell.level === 'below' ? 'text-danger' : cell.level === 'at-or-above' ? 'text-accent' : 'text-foreground';
  return <span className={`inline-flex flex-col items-center gap-0.5 ${tone}`}>
    <span className={`font-semibold tabular-nums ${prominent ? 'text-4xl tracking-tight' : 'text-sm'}`}>
