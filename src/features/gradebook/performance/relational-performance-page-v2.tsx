...
import { Alert, Button, Chip, Drawer, Label, ListBox, Select, Skeleton, Spinner, Tabs } from '@heroui/react';
...
export function RelationalPerformancePageV2({
  ...
  return (
    <section aria-label="Desempenho relacional" className="grid min-w-0 grid-cols-1 gap-4">
      ...
      <div className="grid grid-cols-2 gap-3 sm:max-w-lg" aria-label="Contexto de Desempenho">
        <PerformanceSelect
          id="class"
          label="Turma"
          value={state.filters.classId === null ? 'none' : String(state.filters.classId)}
          disabled={state.busy.classes}
          ...
          onChange={(value) =>
            void state.select({ classId: value === 'none' ? null : Number(value) })
          }
        />
        <PerformanceSelect
          id="period"
      ...
      {state.classes?.nextOffset !== null && state.classes?.nextOffset !== undefined ? (
        <Button
          variant="ghost"
          className="justify-self-start"
          isDisabled={state.busy.classes}
          onPress={() => void state.loadClasses(state.classes!.nextOffset!)}
        >
          Mais turmas
        </Button>
      ) : null}
