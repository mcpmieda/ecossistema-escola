...
function BindingTable({values,onOpen,year}:{values:readonly WorkspaceBindingV2[];onOpen:(value:WorkspaceLinkV2)=>void;year:number}) {
...
function Center({value,onOpen,busy,onMore,year}:{value:WorkspaceCenterV2;onOpen:(value:WorkspaceLinkV2)=>void;busy:boolean;onMore:()=>void;year:number}) {
  ...
  return <Card className="overflow-hidden"><Card.Header className="border-b border-separator bg-surface-secondary/35">
    ...
    </div></div><Chip size="sm" color="accent" variant="soft">{year}</Chip></div>
    ...
    {value.entity.kind==='student'||value.entity.kind==='class-group'?<section>
      ...
      {value.bindings.length?<BindingTable values={value.bindings} onOpen={onOpen} year={year}/>:<p className="text-sm text-muted">Nenhum vínculo encontrado nesta consulta.</p>}
...
export function RelationalWorkspacePageV2() {
  ...
  return <section aria-label="Centrais acadêmicas" className="grid min-w-0 gap-5">
    <header className="flex flex-col gap-3 sm:flex-row sm:items-end sm:justify-between"><div><div className="flex items-center gap-2 text-xs font-semibold uppercase tracking-[0.14em] text-accent"><BookOpenCheck className="size-4" aria-hidden="true"/>Cadastro anual</div><h2 className="mt-2 text-2xl font-semibold tracking-[-0.04em] sm:text-3xl">Cadastros e configuração docente</h2><p className="mt-2 max-w-3xl text-sm leading-6 text-muted">Consulte alunos, turmas, professores, componentes e as ofertas reconhecidas nas planilhas do ano selecionado.</p></div><Chip color="success" variant="soft">Fonte importada · {workspace.year ?? '—'}</Chip></header>
    ...
    {workspace.context?<Card className="overflow-visible"><Card.Content className="grid gap-4 p-4 sm:p-5">
      ...
      <p className="text-xs text-muted">Busca literal, sem unir homônimos. Consulta somente leitura; alterações cadastrais entram pela Importação.</p>
    ...
    {workspace.busy.context?<div role="status" className="flex items-center gap-2 text-sm text-muted"><Spinner size="sm"/>Carregando cadastro de {workspace.year ?? '—'}…</div>:null}
    {workspace.detail && workspace.year!==null?<Center value={workspace.detail} year={workspace.year} onOpen={(value)=>void workspace.open(value)} busy={workspace.busy.detail} onMore={()=>void workspace.open(workspace.detail!.entity,workspace.detail!.nextOffset!)}/>:null}
  </section>;
}
