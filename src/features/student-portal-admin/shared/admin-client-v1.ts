...
export function createPortalAdminClientV1(options: PortalTransportOptionsV1 = {}) {
  ...
  return {
    query: (input: PortalAdminQueryInputV1, signal?: AbortSignal) =>
      send('/api/student-portal/admin/query', responseFor(input.operation), signal, portalRequestBodyV1(adminQueryV1, input)),
    command: (input: PortalAdminCommandInputV1, signal?: AbortSignal) => prepareCommand(input).execute(signal),
    prepareCommand,
  };
}
