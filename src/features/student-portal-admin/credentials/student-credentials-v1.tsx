...
function CredentialsPageV1({
  ...
  return (
    <Card className="pa-credentials-card">
      <Card.Content>
        <h3>
          {scopeLabel || (scope.kind === 'class' ? 'Turma selecionada' : 'Aluno selecionado')}
        </h3>
        {!canWrite && <p>Somente leitura</p>}
        {authorizationLost ? (
          <p role="alert">
          ...
          <>
            <div className="flex items-center gap-2">
              <Chip size="sm" variant="soft">
                <Chip.Label>{data.items.length} alunos</Chip.Label>
              </Chip>
              <Chip size="sm" color="accent" variant="soft">
                <Chip.Label>{chosen.length} selecionados</Chip.Label>
              </Chip>
              ...
              <>
                ...
                <div className="pa-credentials-actions">
                  ...
                  <Button
                    variant="secondary"
                    isDisabled={
                      !canWrite || started || Boolean(read.refreshError) || chosen.length !== 1
                    }
                ...
                <Tooltip>
                  ...
                  <Tooltip.Content>
                    O QR ativo é preservado. PDF compacto com linhas de corte; cópia individual
                    contém somente a imagem.
                  </Tooltip.Content>
                </Tooltip>
              </>
            )}
        ...
        <div className="pa-credentials-actions">
          {!authorizationLost && operationState.state === 'error' && (
            <Button
              variant="secondary"
...
const CredentialSelectionV1 = memo(function CredentialSelectionV1({
  ...
  return (
    <Table className="pa-credentials-table">
      ...
      >
        <Table.Content
          aria-label="Contas para cartões de acesso"
          selectionMode="multiple"
          selectedKeys={selected}
          disabledBehavior="selection"
          disabledKeys={rows
            .filter((row) => !enabled || !eligible.has(row.accountId))
            .map((row) => row.accountId)}
          onSelectionChange={(keys) =>
            onChange(
              new Set(
                keys === 'all' ? eligible : [...keys].map(String).filter((id) => eligible.has(id)),
              ),
            )
          }
        >
          <Table.Header>
            ...
            <Table.Column id="readiness">Acesso</Table.Column>
          </Table.Header>
          <Table.Body>
            {rows.map((item) => (
              <Table.Row key={item.accountId} id={item.accountId}>
                ...
                <Table.Cell>
                  <div className="pa-account-identity">
                    <StudentAvatarV1 id={item.accountId} />
                    <strong>{item.name || 'Nome indisponível'}</strong>
                  </div>
                </Table.Cell>
                ...
                <Table.Cell>
                  <Chip
                    size="sm"
                    color={eligible.has(item.accountId) ? 'success' : 'warning'}
                    variant="soft"
                  >
                    <Chip.Label>{firstAccessLabelV1(item)}</Chip.Label>
                  </Chip>
                </Table.Cell>
              </Table.Row>
            ))}
    ...
    <div className="pa-credentials-feedback" aria-live="polite">
      ...
      {state.state === 'rendering' && (
        <>
          ...
          <ProgressBar
            aria-label="Cartões renderizados"
              ...
              : state.error.state === 'unauthenticated'
                ...
                : state.error.state === 'forbidden'
                  ...
                  : state.error.state === 'conflict'
                    ? 'O estado mudou ou o QR requer uma ação na ficha. Revise a disponibilidade e a seleção.'
                    : state.retryable
                      ? 'A resposta não foi confirmada. Tentar novamente preserva a mesma solicitação.'
                      : 'Solicitação recusada. Revise a disponibilidade e a seleção.'}
          </p>
          {state.retryable && (
      ...
      {state.state === 'cancelled' && (
        <p>Arquivo descartado. O QR continua válido para reimpressão.</p>
      )}
      {state.state === 'expired' && (
        <p>Arquivo temporário expirado. Prepare outra cópia quando precisar.</p>
      )}
    </div>
  );
}
