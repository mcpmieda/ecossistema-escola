...
export async function jobPolicyVersionV1(tx: StudentPortalPostgresQueryV1, accountId: string): Promise<string> {
  ...
  const row = rows[0];
  // 7 fields before migration 0020 (#1132), 8 after it.
  if (!row || (row.fields !== 7 && row.fields !== 8) || row.epochs !== 1) throw new Error('student-portal-policy-defaults-unavailable');
