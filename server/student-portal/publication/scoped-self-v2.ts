...
import { dataVectorV1, parseDataVectorV1, PERIODS_V1, publicationDigestV1, type PublicationRowV1 } from './state-v1';
...
const hasFacts = (period: SubjectV2['periods'][number]) => period.final.kind !== 'absent'
  || Boolean(period.partials?.some((partial) => partial.mark.kind === 'score'));

/** Auth/session/current-link validation is owned by the caller. Only this student's prepared rows are loaded. */
export async function scopedSelfV2(tx: StudentPortalPostgresQueryV1, context: ContextV2,
  previous: SelfResponseV1 | null, legacy: readonly PublicationRowV1[], requestId: string): Promise<SelfResponseV1> {
    ...
    if (row.payload_json !== null && row.edition_class_id === context.policy.classId) {
      ...
      if (!source) {
        const projected = reader.projectPreparedSourceV2(link, revision, {
          ...z.record(z.string(), z.unknown()).parse(row.payload_json), account_id: accountId, data_version: revision,
        });
      ...
      if (!finalSource || BigInt(source.student.dataVersion.split(':')[1]!) > BigInt(finalSource.student.dataVersion.split(':')[1]!))
        finalSource = source;
      accepted[index] = revision;
    } else if (row.payload_json === null && previous && oldVector[index] === revision
  ...
  const finalAuthority = !explicitWithdrawal && Boolean(sameState && (finalSource?.finalAuthority.global
    || (!finalSource && usedLegacy && accepted.some((revision) => revision !== null))));
  const finalSubjects = finalSource ? academicToSelfV1(finalSource.student, accountId).subjects : previous?.subjects ?? [];
  const ordered = [...subjects.values()]
    .sort((a, b) => compareSourceSubjectPresentationV1(a.label, b.label) || a.subjectId - b.subjectId)
    .map((subject, order) => {
      const outcome = finalSubjects.find((item) => item.subjectId === subject.subjectId)?.officialOutcome;
      const hasAuthority = finalSource ? finalSource.finalAuthority.subjectIds.includes(subject.subjectId) : usedLegacy;
      return { ...subject, order, periods: subject.periods.sort((a, b) => PERIODS_V1.indexOf(a.period) - PERIODS_V1.indexOf(b.period)),
        ...(finalAuthority && hasAuthority && outcome !== undefined ? { officialOutcome: outcome } : {}) };
    });
  const projection = selfResponseV1.parse({ contractVersion: 1, requestId, state: ordered.length ? 'ready' : 'no-publication',
    profile: { ...context.profile, result: finalAuthority ? finalSource?.student.profile.result ?? previous!.profile.result : context.profile.result },
    subjects: ordered, generatedAt: context.now.toISOString(), revisions: {
      dataVersion: dataVectorV1(accepted), policyVersion: context.policy.policyVersion,
      publicationVersion: `pub:${publicationDigestV1(rows.map((row) => [row.period, row.target_revision, String(row.decision_version)]))}`,
    } });
  return applyPublishedVisibilityV1(projection, context.policy.settings.value, context.now, finalAuthority);
}
