...
export async function recordResetWriteV1(
  ...
  const result = await transaction
    .prepare(
      ...
      ?::uuid,?::smallint,?::text,false,'{}'::integer[],statement_timestamp())`,
    )
    .bind(crypto.randomUUID(), year, cause)
...
export async function recordImportResetWriteV1(
  ...
  if (typeof buffered.afterImportFlush === 'function') {
    buffered.afterImportFlush((database) => recordResetWriteV1(database, year, cause));
  } else {
    await recordResetWriteV1(transaction, year, cause);
  }
}
