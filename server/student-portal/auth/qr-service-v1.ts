...
export class QrServiceV1 {
  ...
  async command(actorId: string, input: unknown) {
    ...
    return authTransactionV1(this.sql, async (tx, store) => {
      ...
      } else if (operation.startsWith('qr-') && active)
        qr = await this.url(active.credentialId, active.keyVersion);
      if (resetPassword)
        await tx.unsafe(
      ...
      await authAuditV1(
        store,
        next,
        command.operation === 'block' && !command.blocked ? 'unblocked' : kinds[operation],
        now,
