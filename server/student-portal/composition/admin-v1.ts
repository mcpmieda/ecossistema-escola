...
export async function portalAdminRpcV1(env: PortalCompositionEnvV1, kind: 'query' | 'command', context: unknown, input: unknown) {
  ...
  try {
    ...
    return await portalDatabaseV1(env, kind === 'query' ? 'admin-query' : 'admin-command', (sql) =>
      // V2 is read-only and sets transaction isolation before its first query.
      // The audit SET LOCAL wrapper is retained for every existing V1 path.
      new PortalAdminApiV1(kind === 'query' && parsed.data.contractVersion === 2 ? sql : withAuditSqlV1(sql, trusted.data.clientIp ?? null), { ...keys, tenantId: env.PORTAL_ADMIN_TENANT_ID })[kind](trusted.data, parsed.data));
