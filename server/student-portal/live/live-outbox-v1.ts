...
async function claimLiveEventsV1(sql: StudentPortalPostgresSqlV1, token: string, limit: number) {
  return sql.begin(async (tx) => tx.unsafe(`WITH pending AS (
    ...
    RETURNING event.id::text,event.audience,event.domain,event.version,event.account_id,
      event.class_id,to_json(event.student_ids) AS student_ids,event.occurred_at`, [limit, token]));
}
...
export async function dispatchPortalLiveEventsV1(env: PortalCompositionEnvV1, limit = 50): Promise<number> {
  ...
  try {
    ...
    if (!claimed) { outcome = 'ok'; return 0; }
    ...
    if (failed.length || acknowledged !== accepted) throw new Error('student-portal-live-delivery-unavailable');
    outcome = 'ok';
    return acknowledged;
  } finally {
    if (claimed || outcome === 'unavailable') {
      // Fixed aggregate diagnostic only: no routing identifiers, SQL, tokens or raw errors.
      console.info(JSON.stringify({ event: 'student-portal-live-drain-v1', outcome,
        occurredAt: new Date().toISOString(), elapsedMs: Math.max(0, Date.now() - started),
        claimed, accepted, acknowledged, rejected }));
    }
  }
}
