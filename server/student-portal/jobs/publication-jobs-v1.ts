  ...
  async perform(job: ClaimedJobV1): Promise<'done' | 'stale' | 'deferred' | 'failed'> {
    try {
      return await accountTransactionV1(this.sql, async (tx, store) => {
        ...
        if (
          jobPublicationVersionV1(mask, rows) !== job.publicationVersion ||
          (await jobPolicyVersionV1(tx, job.accountId)) !== job.policyVersion ||
          (mask !== 0 && (await currentRevisionV1(tx)) !== job.dataVersion)
        )
        ...
        if (mask === 0) {
          ...
          if (
            !(await store.swapProjection(job.accountId, projection, projection.revisions)) ||
            !(await finishJobV1(tx, job, await authNowV1(tx)))
          )
            throw new Error('student-portal-publication-stale');
          return 'done';
        }
        ...
        if (
          !due ||
          Date.parse(context.policy.settings.value.calendar.yearEndsAt!) <= context.now.getTime()
        )
        ...
        const source = await new AcademicStudentReaderPostgresV1(
          tx,
        ).readPublicationSourceInTransaction(tx, context.account.link!, job.dataVersion);
        ...
        let projection = applyPublishedVisibilityV1(
          selfResponseV1.parse({
            contractVersion: 1,
            requestId: crypto.randomUUID(),
