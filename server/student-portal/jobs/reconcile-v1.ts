...
export class PublicationReconcilerV1 {
  ...
  async run(limit = 20) {
    ...
    for (const accountId of ids) {
      try {
        ...
        failed++;
      } // No source text or student data in telemetry.
    }
  ...
  private async reconcileAccount(accountId: string) {
    await accountTransactionV1(this.sql, async (tx, store) => {
      ...
      const observed = await tx.unsafe(
        'SELECT id FROM student_portal.publication_job WHERE account_id=$1::uuid AND policy_version=$2 LIMIT 1',
        [accountId, policyVersion],
      );
      let source = null;
      try {
        source = await new AcademicStudentReaderPostgresV1(tx).readOfficialInTransaction(
          tx,
          context.account.link!,
          revision,
        );
      } catch {
        /* Malformed source cannot replace a valid published snapshot. */
      }
      for (const period of PERIODS_V1) {
        const prior = before.find((row) => row.period === period)!;
        const delivered =
          previous && vector[PERIODS_V1.indexOf(period)] === prior.publishedRevision
            ? prior.publishedRevision
            : null;
        const available = Boolean(
          source?.subjects.some((subject) =>
            subject.periods.some(
              (item) =>
                item.period === period &&
                (item.final.kind !== 'absent' ||
                  item.partials?.some(
                    (partial) => partial.mark.kind === 'score' || partial.notDone === true,
                  )),
            ),
          ),
        );
        const state =
          delivered !== null
            ...
            : available
              ...
              : 'no-data';
        await tx.unsafe(
          `INSERT INTO student_portal.publication(scope_key,scope_kind,academic_year,account_id,period,state,
      ...
      const mask = rows.reduce(
        (bits, row) => bits | (row.publishedRevision !== null ? publicationMaskV1(row.period) : 0),
        0,
      );
      const jobs: JobInputV1[] = [];
      if (
        mask &&
        (context.policy.settings.value.autoUpdate ||
          (observed.length === 0 &&
            rows.every(
              (row) => row.publishedRevision === null || row.publishedRevision === revision,
            )))
      ) {
        jobs.push({
          accountId,
          ...
          policyVersion,
          publicationVersion: jobPublicationVersionV1(mask, rows),
        });
      }
      ...
      const pending = await tx.unsafe(
        `SELECT data_version,policy_version,publication_version FROM student_portal.publication_job
        WHERE account_id=$1::uuid AND state IN ('queued','running','failed') AND policy_version<>$2 AND data_version=$3`,
        [accountId, policyVersion, revision],
      );
      for (const item of pending) {
        ...
        if (pendingMask && item.publication_version === jobPublicationVersionV1(pendingMask, rows))
          jobs.push({
            ...
            dataVersion: revision,
            policyVersion,
            publicationVersion: jobPublicationVersionV1(pendingMask, rows),
          });
      }
      if (observed.length === 0 && jobs.length === 0)
        jobs.push({
          accountId,
          dataVersion: revision,
          policyVersion,
          publicationVersion: jobPublicationVersionV1(0, rows),
        });
