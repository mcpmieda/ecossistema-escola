...
type TransactionDatabase = D1WriteDatabaseV1 & { transaction<T>(operation: (db: D1WriteDatabaseV1) => Promise<T>): Promise<T> };
const fail = (state: PerformanceFailureV2): PerformanceResponseV2 => ({ transportVersion: 2, state });
const integer = (value: unknown): number => {
  if (typeof value !== 'number' || !Number.isSafeInteger(value)) throw new Error('invalid-performance-row');
  return value;
};
const nullable = (value: unknown): number | null => value === null || value === undefined ? null : integer(value);
...
export async function readRelationalPerformanceV2(db: D1WriteDatabaseV1, request: PerformanceRequestV2, options: {
  ...
  const common = { transportVersion: 2, state: 'ready', authority: 'calculated-preview',
    context: { year: integer(year.ano), minimumApprovalMilli: integer(year.minimo_aprovacao), maxCouncilComponents: integer(year.max_componentes_conselho) }, readAt: text(year.read_at) } as const;
  if (request.operation === 'classes') {
    const rows = await all(db, `SELECT id, codigo FROM gradebook.turma WHERE ano = ? ORDER BY codigo COLLATE "C", id LIMIT ? OFFSET ?`, [request.year, request.limit + 1, request.offset]);
  ...
  const offers = (await all(db, `SELECT o.id,o.disciplina_id,d.nome AS disciplina_nome,o.professor_id,p.nome AS professor_nome
  ...
  if (students.length && offers.length) {
    ...
    for (const row of facts) {
      ...
      if (row.slot !== null) {
        ...
        group.facts.push({ term: integer(row.trimestre) as SimplifiedAcademicTermV1, slot,
          maximumMilli: nullable(row.maximo), valueMilli: nullable(row.valor),
          label: typeof row.descricao === 'string' && row.descricao.trim() ? row.descricao : slot === 1 ? 'I Avaliação' : slot === 2 ? 'II Avaliação' : slot === 3 ? 'Prova paralela' : `Atividade ${slot - 10}` });
      }
      grouped.set(key, group);
    }
  }
  const project = (value: Row) => offers.map((offering) => {
    const group = grouped.get(`${integer(value.id)}:${offering.id}`);
    return projectPerformanceFactsV2(offering.id, group?.facts ?? [], group?.closing ?? EMPTY_PERFORMANCE_CLOSING_V2, common.context.minimumApprovalMilli);
  });
  ...
  if (request.operation === 'student-detail') {
    ...
    return { ...common, ...selected, operation: 'student-detail', offers,
...
export function createRelationalPerformanceV2(database: D1WriteDatabaseV1) {
  return { async execute(input: unknown): Promise<PerformanceResponseV2> {
