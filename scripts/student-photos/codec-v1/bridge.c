...
/* Private bounded bridge. The caller owns a fresh instance per operation. */
...
static int write_bounded(const uint8_t* data, size_t size, const WebPPicture* picture) {
  ...
  if (size > budget->limit - output_size) {
    budget->exceeded = 1;
    return 0;
  }
 ...
 * 6=output budget, 7=allocation/ABI. No resize or automatic quality retry. */
int photo_normalize(size_t length, int variant, int quality) {
  output_size = 0;
  result_width = result_height = 0;
  wipe(output, sizeof(output));
  if ((variant != 0 && variant != 1) || (quality != 92 && quality != 86 && quality != 80)
      || length < 20 || length > (variant == 0 ? CAPACITY : CAPACITY / 2)) return 1;
  ...
  const size_t pixels_size = (size_t)width * (size_t)height * 4;
  ...
  picture.writer = write_bounded;
  picture.custom_ptr = &budget;
  const int imported = WebPPictureImportRGBA(&picture, pixels, width * 4);
