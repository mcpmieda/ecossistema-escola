...
Segurança: o cliente usa o Durable Object existente com propósito `security`. Recebe apenas confirmação da conexão ou aviso mínimo de reautorização e consulta `/session` vinculada à conta exibida. Não consulta `/me` em resposta a eventos. Notas novas exigem atualização manual. Ao retornar do histórico, a página limpa o conteúdo protegido e o recarrega após nova verificação de sessão (login estável, 22/09/2026 — ver `DECISIONS.md`).
