...
- **R2 (substituída em 24/09/2026 por decisão do responsável): o relatório só aparece junto com a nota.** O fechamento de um trimestre só é lido quando as notas desse trimestre estão liberadas para a turma (períodos permitidos + datas de divulgação, a mesma regra das notas). Trimestre encerrado mas não liberado não gera fechamento, resumo nem aba. Regra anterior (fechamento sozinho antes da liberação) foi descartada após a turma 8A ver uma aba do 2º trimestre sem notas.
