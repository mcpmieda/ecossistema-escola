...
P1 está tecnicamente encerrada em PR741; P2 foi autorizada para execução sequencial, preservando identidades CODEX/CHAT ONLINE. A preparação #743 consolidou a documentação; #744–#756 implementaram os contratos e componentes. #757 integra telas e serviços, sem habilitar acesso escolar. As decisões de produto v3.0 + Adendo01 permanecem: Portal2026, fonte BN, nascimento manual, calendário administrável, navegação ADM atual, sem correspondência externa/foto/rede social. O comando histórico do Word para planejar somente P1 não governa o início de P2 agora autorizado.
...
#745 contratou e #747 integrou o helper acadêmico único: valor/máximo versus mínimo anual oficial no mesmo snapshot, com comparação exata BigInt. Máximo ausente mantém meetsMinimum:null; não há arredondamento, clamp nem mínimo fixo na UI. #746 integrou accounts-read/overview V2, catálogo completo e último acesso; #756 acrescentou sessions-read V2 com validade efetiva e contexto compartilhado. A composição #757 consome esses deltas, sem criar regra acadêmica ou DDL.
...
- Calendário: enrollmentStartsAt, yearStartsAt (=inícioT1), t1EndsAt, t2StartsAt/t2EndsAt, t3StartsAt/t3EndsAt, recoveriesStartAt, yearEndsAt, finalDisclosureAt e disclosure. Fuso America/Sao_Paulo; timestamps RFC3339 com offset, instantes UTC na persistência; precisão em segundos. Início inclusivo, fim exclusivo; instante de fim é a primeira hora proibida. Datas não são presumidas31/12. Os inícios separados representam recessos entre períodos; calendários anteriores sem esses campos continuam usando o limite precedente como início. Uma data nula de divulgação dos períodos delega a liberação ao comando manual de publicação e não bloqueia o Portal. finalDisclosureAt nulo mantém somente o resultado final oculto. Modo single (at+períodos abrangidos) ou per-period com seis datas; modo inativo não mantém job. Datas passadas/efeito imediato exigem confirmação. Ordem dos marcos principais validada; data final própria não é inventada.
...
- API ADM completa: contas/primeiro acesso/filtros/turma; QR emissão/reprint/regenerate/lote; reset senha/conta; bloquear/desbloquear; sessões individual/conta/turma; nascimento; configurações/herança; população escolar idempotente; publicação; auditoria; saúde; preview/encerramento de vínculos. Confirmação+CAS+idempotência em mutações.
