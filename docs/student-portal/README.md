...
`npm run verify` cobre lint, tipos, testes, builds e provas workerd. Para executar somente o runtime Workerd, rode primeiro `npm run build:student-portal` e depois `npm run test:student-portal-runtime`; a suíte agora falha com mensagem explícita quando os bundles locais estão ausentes. `npm run test:student-portal-postgres` exige PostgreSQL local descartável em loopback, banco `portal705_test` e `PORTAL_TEST_DATABASE_URL`; não use produção nem um banco já semeado. As suítes nativas cobrem runtime, smokes, fundação, consultas administrativas e composição. Fixture e selo sintético não provam Entra real, dispositivo ou piloto legítimo.
