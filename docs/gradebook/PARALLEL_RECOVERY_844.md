# Recuperação paralela — #844, retificada pela #850 / BN-DEC-035

## Regra e precedência

A #844 foi implementada sobre `main@8376682e1582a7563999b25f6dea3e64c73f39ab`. A soma aditiva descrita naquela entrega foi uma interpretação equivocada do executor, não a intenção confirmada pelo responsável. Em 18/09/2026, 09:39–09:57 UTC, a **#850 / BN-DEC-035** retificou a composição e estabeleceu a exceção de nota numérica lançada. A regra vigente abaixo prevalece; detalhes em [PARALLEL_QUANTITATIVE_850.md](PARALLEL_QUANTITATIVE_850.md).
...
É normalmente elegível somente quando **Q está abaixo do limite quantitativo E Q + L está abaixo do limite total**. Comparar valores exatos em milésimos antes de qualquer arredondamento e antes de aplicar P. Igualdade com qualquer limite dispensa a paralela. A soma dos máximos configurados continua disponível para diagnóstico, mas não redefine esses limites institucionais. A única exceção é **P numérica registrada, inclusive zero**, que deve ser respeitada mesmo sem elegibilidade normal; vazio não aciona a exceção.

Quando P > Q, **quantitativo considerado = P** e **total bruto = P + L**. Caso contrário, quantitativo considerado = Q e total bruto = Q + L. Nunca somar Q e P entre si. O quantitativo original continua Q para diagnóstico; o ganho é max(P-Q,0), não P inteiro como bônus. A recuperação não muda o máximo quantitativo. O resultado posterior não pode ser usado circularmente para decidir a elegibilidade anterior.

Exemplo exclusivamente sintético: Q = 0,2 (uma avaliação ausente), L = 12 e P = 7 em T2. O total anterior é 12,2 e permanece abaixo de 18; P supera Q; soma bruta = 19 e nota arredondada = 19. O exemplo confirmado pelo responsável Q=4, P=7, L=10 resulta em 17. Não são registros acadêmicos reais.

## Núcleo, cobertura e consumidores

`resolveSimplifiedTermV1` permanece a única implementação da regra. `rawMilli` contém a soma exata e `roundedMilli` mantém a função homologada: fração abaixo de 0,300 desce ao inteiro; de 0,300 até antes de 0,750 vai a 0,5; a partir de 0,750 vai ao próximo inteiro. A #850 não muda essa função.
...
**Cobertura #848, complementada pela #850:** a PARA existente participa quando normalmente elegível ou numericamente registrada. Nota zero ou outro número resolve o instrumento; valor ausente mantém a pendência somente para elegíveis. Sem elegibilidade e sem nota, não aparece nem causa asterisco. Não fez exige vazio observado; zero real aparece como Tirou zero. Outros instrumentos pendentes não são resolvidos por essa alteração. Mapa de consumidores, edição aprovada do Portal e regressões em [PARALLEL_VISIBILITY_848.md](PARALLEL_VISIBILITY_848.md).
...
- Regressões anteriores são atualizadas apenas onde afirmavam a regra substituída; gates de REC, arredondamento, isolamento, autorização, limites e persistência permanecem. `performance/parallel-quantitative-850.test.ts` cobre a retificação, a exceção e os consumidores.
