...
A auditoria produtiva de 18/09/2026 encontrou, após a remoção dos fatos acadêmicos de 2025, diagnósticos e revisões técnicas ainda atribuíveis ao ano. A regra vigente passa a ser: um reset anual concluído não conserva `revision_event`, `year_reset_preview_proof` nem `academic_revision` do ano removido. O contrato HTTP V1 e `deletedRows` permanecem compatíveis e continuam descrevendo a exclusão acadêmica; a limpeza técnica é uma pós-condição interna obrigatória. Contas e histórico de identidade Portal não são apagados por essa mudança e vínculos vivos continuam bloqueando o reset.
