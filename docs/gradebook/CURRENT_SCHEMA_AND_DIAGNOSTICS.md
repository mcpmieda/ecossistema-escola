...
`0005` é a extensão proposta pela #654 e ainda não integra a fotografia produtiva deste checkpoint: adiciona apenas `boletim_snapshot` (13 colunas e 2 índices), sem backfill. Sua aplicação exige backup/preflight/CI/revisão próprios e um postflight novo; presença do arquivo no Git não muda a contagem oficial de 28 tabelas.
