...
| `AcademicStudentReaderPostgresV1.projectPreparedSourceV2` | Filtra as parciais da mesma edição aprovada pela elegibilidade do núcleo, preservando zero, observado/null e a AM/U oficial. |
...
- `performance/parallel-visibility-848-sql.test.ts`: leitores PostgreSQL/PGlite reais, seis instruções por matriz/detalhe/análise, estado/granularidade, fato armazenado preservado e prévia de Boletim sem escrita em snapshots.
