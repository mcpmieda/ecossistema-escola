...
A #636 foi integrada em `main@4d8256fa6f741f4fb0b6ade8676d0f9193b7a460`; deploy 254 aprovado. Planejamento reconciliado, leituras em lote, baseline de schema reproduzível e Auditoria transacional fazem parte dessa base. Deploy não equivale a smoke autenticado ou aceite integral das telas.

A entrega seguinte é **#639 / PR #640**, branch `feat/bn-final-1-centrais-relacionais`: contexto, pesquisa e quatro Centrais somente leitura no transporte V2. O código e a interface estão na branch até integração/publicação próprias. Desempenho, Conselho, Boletins, Relatórios e escritas cadastrais ainda precisam de adaptação. Nome `d1-*` não demonstra provider físico.

## Leitura e execução

1. [COMECE_AQUI.md](COMECE_AQUI.md): tarefa e dependências.
2. [PROJECT_STATE.yaml](PROJECT_STATE.yaml): integrado/publicado versus branch.
...
| Fase | Issue | Entrega |
| --- | --- | --- |
| FINAL-1 | #633; bloco #639/#640 | documentação, baseline, fontes e consumidores relacionais |
...
Fluxo: issue → branch → PR → `npm run verify` → CI → integração/publicação autorizadas → evidência sanitizada. Nenhuma migration, alteração de infraestrutura ou ativação acadêmica está autorizada por este documento.
