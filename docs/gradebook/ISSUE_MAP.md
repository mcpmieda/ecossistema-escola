...
- **Integração atual:** [#237 — sexta onda](https://github.com/mcpmieda/ecossistema-escola/issues/237)
- **Site oficial:** `https://admin.escolaieda.com/#/banco-de-notas`
- **Onda atual:** #234, #235 e #236, simultaneamente.
- **Ondas concluídas:** primeira à quinta.
- **Armazenamento aprovado:** Cloudflare D1, conforme #200.
- **D1 atual:** schema/migrations locais integrados; nenhum banco ou binding de produção criado.
...
| F3 — Motor nativo | [#186](https://github.com/mcpmieda/ecossistema-escola/issues/186) | Em construção | 3/7 | Comparação fonte × cálculo nativo |
| F4 — Reconciliação e Auditoria | [#187](https://github.com/mcpmieda/ecossistema-escola/issues/187) | Em construção | 3/6 | Revisão, pendências e promoção do lote |
...
| [#226](https://github.com/mcpmieda/ecossistema-escola/issues/226) | Composição trimestral nativa 30/30/40 e 45/55 | #231 / `5780f88` |
| [#227](https://github.com/mcpmieda/ecossistema-escola/issues/227) | Schema D1 V1: 2 migrations, 19 tabelas e índices | #233 / `781a2a2` |
| [#228](https://github.com/mcpmieda/ecossistema-escola/issues/228) | Planejador idempotente e incremental de reimportação | #232 / `9b4e4fa` |

O conjunto da quinta onda foi publicado pelo deploy `33436989871`. Não houve nova tela nessa onda.

## Onda 6 — pronta para agentes

As três issues abaixo escrevem em áreas separadas e podem ser executadas ao mesmo tempo:

1. [#234 — recuperação paralela nativa V1](https://github.com/mcpmieda/ecossistema-escola/issues/234)
   - Calcula aplicabilidade pelo quantitativo abaixo de 60% do próprio máximo.
   - Preserva nota original, paralela, maior valor, ganho, cobertura e motivo.

2. [#235 — catálogo de streams por fonte lógica e leitura D1 local](https://github.com/mcpmieda/ecossistema-escola/issues/235)
   - Adiciona migration 0003 e a relação explícita entre fonte lógica e registros acadêmicos.
   - Implementa as leituras D1 necessárias ao planejador, sem binding ou recurso remoto.

3. [#236 — executor transacional do plano de reimportação](https://github.com/mcpmieda/ecossistema-escola/issues/236)
   - Aplica apenas itens novos/alterados contra as portas.
   - Comprova commit, conflito e rollback em memória; não acessa D1 diretamente.

O integrador acompanha os três trabalhos em [#237](https://github.com/mcpmieda/ecossistema-escola/issues/237).

## Lacuna de integração explicitamente rastreada

O planejador da #228 precisa listar os streams atuais pertencentes a uma fonte lógica para detectar registros que desapareceram da nova planilha. O schema inicial da #227 não possui essa associação em tabela própria.

A correção oficial é a #235. Não é permitido usar nome de arquivo como identidade nem varrer `payload_json` para esconder essa lacuna.

## Próximas dependências conhecidas

| Tarefa futura | Liberação necessária |
|---|---|
| Consolidação trimestral completa/próxima regra do motor | #234 integrada |
| Adaptador D1 de escrita e transação física | #235 + #236 integradas |
| Binding D1 local/preview e aplicação de migrations fora dos testes | adaptador de escrita aceito e issue de infraestrutura própria |
...
#234 + #235 + #236 ──> integração #237

#235 + #236 ──> adaptador D1 de escrita/promoção
```
...
O operador já confirmou o happy path da #199 com dois arquivos XLSB: reconhecimento, SHA-256 abreviado, manifesto e leitura acadêmica apareceram corretamente. Ainda faltam expandir o hash completo, observar a etapa transitória e conferir uma falha isolada para encerrar o smoke completo.
