...
- **Quantitativo:** parcela considerada pelo núcleo na composição. Q original é somente AV1+AV2; conforme BN-DEC-035/#850, uma PARA superior substitui Q, nunca se soma a ele. O máximo quantitativo não aumenta. A elegibilidade normal usa os dois limites institucionais de 60%, antes de PARA/arredondamento e sem exigir notas nas AV; P numérica registrada, inclusive zero, é a única exceção. A comparação pedagógica das duas AV no painel de aprendizagem continua usando Q original, enquanto esta lente de composição usa o quantitativo considerado. Detalhes e precedência em [PARALLEL_QUANTITATIVE_850.md](PARALLEL_QUANTITATIVE_850.md).
...
Nas lentes de composição, o modo Recuperação mantém a população e a aplicabilidade por componente resolvidas pelo núcleo, mas apresenta a **composição das notas regulares**. REC não possui uma decomposição inventada em avaliações/qualitativo. A interface explica isso; Resultado continua mostrando REC/N-C. Desde a #850, PARA numérica é respeitada mesmo sem elegibilidade normal e participa do recorte; só a célula sem elegibilidade e sem nota permanece não aplicável. Mostrar P não significa ganho: P menor ou igual a Q conserva Q. Nenhum cálculo é repetido na interface.
